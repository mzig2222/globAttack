...
public class GlobAttack extends PApplet {
    ...
    Player user = new Player(5, 5, 1, 1, 0);
        ...
        switch (currentState) {
            ...
            case RUNNING:
                ...
                for (int i = 0; i < enemies.size(); i++) {
                    ...
                    if ((abs(playerX - en.x) < 15 && abs(playerY - en.y) < 15) && (millis() - healthTime >= 3000)) {
                        user.health--;
    ...
    public void drawGameOver() {
        ...
        text("High Score: " + user.highScore, width / 2, height / 2 + 10);
        image(restartButton, width / 2, height / 2 + 165);
        fill(255, 255, 255);
        text("Increase health", width / 2, height / 2 + 170);
        image(restartButton, width / 2, height / 2 + 110);
        fill(255, 255, 255);
        text("Increase bullet speed", width / 2, height / 2 + 115);
        image(restartButton, width/2, height/2 + 55);
        fill(255, 255, 255);
        text("Increase player speed", width / 2, height / 2 + 60);
    }
    ...
    public void mousePressed() {
        switch (currentState) {
            case RUNNING:
                ...
                float vx = user.bSpeed * cos(angle);
                float vy = user.bSpeed * sin(angle);
                bullets.add(new Bullet(playerX, playerY, vx, vy));
                break;
            case OVER:
                if (mouseX > (width / 2 - 135) && mouseX < (width / 2 + 135) && mouseY > height / 2 + 100 - 75 && mouseY < (height / 2 + 100 + 75)) {
                    for (int i = 0; i < enemies.size(); i++) {
                        enemies.remove(i);
                        score = 0;
                        enemySpeed = 1f;
                    ...
                    if (mouseY > (height / 2 + 100 + 25) && mouseY < (height / 2 + 100 + 75)) { //increase health
                        user.maxHealth++;
                        System.out.println("health increased to " + user.maxHealth);
                    } else if (mouseY > height / 2 + 100 - 25 && mouseY < (height / 2 + 100 + 25)) {  //increase bullet speed
                        user.bSpeed += 5;
                        System.out.println("bullet speed increased to " + user.bSpeed);
                    } else if (mouseY > height / 2 + 100 - 75 && mouseY < (height / 2 + 100 - 25)) { //increase player speed
                        user.pSpeed += 5;
                        System.out.println("player speed increased to " + user.pSpeed);
                    }
    ...
    class Player{
        ...
        float health;
        int highScore;
        Player(float bSpeed, float pSpeed, float maxHealth, float health, int highScore){
            this.bSpeed = bSpeed;
            this.pSpeed = pSpeed;
    ...
    class Boss{
        ...
        float maxHealth = 10;
        float health;
        Boss(float bSpeed, float cSpeed, float maxHealth, float health){
            this.bSpeed = bSpeed;
            this.cSpeed = cSpeed;
            this.maxHealth = maxHealth;
            this.health = health;
